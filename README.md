...
面向 AstrBot 的 Minecraft Java 版服务器管理与长期记忆插件。除服务器配置、状态查询、在线人数趋势和 Web 管理界面外，还集成 LivingMemory 的对话总结、混合检索、图记忆、自动遗忘和 Agent 记忆工具。
...
- 可选启用 LivingMemory，自动总结会话并通过 BM25、向量和图记忆进行长期召回
- 提供 `recall_long_term_memory` 与可选的 `memorize_long_term_memory` Agent 工具
- 提供长期记忆状态、检索、删除、索引重建、会话总结和清理命令
...
若已安装独立的 `astrbot_plugin_livingmemory`，必须先停用该旧插件，避免重复注册 `/lmem` 命令、事件钩子和 Agent 工具。合并插件会继续使用 `StarTools.get_data_dir('astrbot_plugin_livingmemory')` 对应的数据目录；不要同时运行新旧两个插件。
...
LivingMemory 管理命令均需要 AstrBot 管理员权限：

| 命令 | 参数 | 说明 |
| --- | --- | --- |
| `/lmem status` | 无 | 查看长期记忆状态 |
| `/lmem search` | `关键词 [k]` | 检索记忆，`k` 默认 5 |
| `/lmem forget` | `记忆 ID` | 删除指定记忆 |
| `/lmem rebuild-index` | 无 | 重建文档检索索引 |
| `/lmem rebuild-graph` | 无 | 重建图记忆索引 |
| `/lmem webui` | 无 | 查看页面入口说明 |
| `/lmem summarize` | 无 | 立即总结当前会话 |
| `/lmem reset` | 无 | 重置当前会话记忆上下文 |
| `/lmem cleanup` | `[preview\|exec]` | 预览或执行历史注入片段清理 |
| `/lmem help` | 无 | 查看 LivingMemory 帮助 |

## LivingMemory 配置、工具与页面

长期记忆仅在根插件配置中存在 `living_memory` 节且 `living_memory.enabled` 为 `true` 时启动。Embedding 与总结模型可在 `living_memory.provider_settings` 中指定；留空时由 LivingMemory 按 AstrBot 当前 Provider 配置处理。其余召回、总结、图记忆、衰减、备份与清理参数均位于同一配置节。

启用后可按配置注册以下 Agent 工具：

- `recall_long_term_memory`：主动检索长期记忆，默认开启
- `memorize_long_term_memory`：主动写入长期记忆，默认关闭

页面入口位于 AstrBot Dashboard 的“插件 -> 周易的个人插件 -> Pages -> `livingmemory-dashboard`”。Minecraft 页面仍使用 `mc-manager`。LivingMemory 页面 API 使用根插件命名空间，旧独立插件必须保持停用。
...
Minecraft 数据继续使用根插件 `astrbot_zhouyi_plugin` 的数据目录。LivingMemory 则固定复用旧插件名 `astrbot_plugin_livingmemory` 的数据目录，因此集成不会另建一套长期记忆数据位置。启动导入阶段会在根插件配置不存在时尝试将旧 LivingMemory 配置包装到 `living_memory` 节；旧配置文件会保留。

这里仅说明目录和配置兼容策略，不代表任何具体环境中的真实数据已经完成迁移或验证。操作前仍应自行备份长期记忆数据目录。
...
├── livingmemory/               # LivingMemory 组合组件与长期记忆核心
├── script/
...
├── pages/livingmemory-dashboard/ # LivingMemory 官方插件页面
...
仓库根许可证见 [LICENSE](LICENSE)。其中集成的 LivingMemory 代码来源于作者 **lxfight** 的 **LivingMemory 2.3.6**，上游仓库：`https://github.com/lxfight-s-Astrbot-Plugins/astrbot_plugin_livingmemory`，上游采用 **AGPL-3.0**。使用、修改和分发该部分代码时应遵守其上游许可证及归属要求。

## 致谢

服务器信息查询和图片展示功能基于早期 `mcgetter` 实现继续修改；长期记忆能力来自 LivingMemory 上游项目。感谢原作者及相关开源项目。
