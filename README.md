...
面向 AstrBot 的 Minecraft Java 版服务器管理与长期记忆插件。除服务器配置、状态查询、在线人数趋势和 Web 管理界面外，还集成 Memory 后端的对话总结、混合检索、图记忆、自动遗忘和 Agent 记忆工具。
...
长期记忆管理命令均需要 AstrBot 管理员权限；同时支持 `/zhouyi memory <子命令>`：
...
| `/lmem help` | 无 | 查看长期记忆帮助 |

## Memory 配置、工具与页面

长期记忆仅在根插件配置中存在 `memory` 节且 `memory.enabled` 为 `true` 时启动。Embedding 与总结模型可在 `memory.provider_settings` 中指定；留空时由 Memory 后端按 AstrBot 当前 Provider 配置处理。旧独立配置和根 `living_memory` 节会在启动导入阶段原子迁移，现有 `memory` 字段优先，未知字段保留。
...
Minecraft 数据与长期记忆数据统一位于根插件 `astrbot_zhouyi_plugin` 数据目录，Memory 运行时只使用其中的 `memory/` 子目录。首次启动会以只读方式从旧独立插件数据目录迁移：SQLite 使用 backup API 复制，并通过哈希、`quick_check`、`integrity_check` 和核心表计数验证；目标写入 `READY` 状态后才启用 Memory。迁移失败只停用长期记忆，Minecraft 功能继续运行。

迁移使用同目录 staging 和原子切换，源目录不会删除或写入；目标已处于 `READY` 时重复启动不会再次复制。
...
```text
.
├── main.py                     # AstrBot 插件入口、事件与命令路由
├── runtime.py                  # Memory、页面、独立服务和趋势任务生命周期
├── web_api.py                  # Plugin Page 后端 API
├── standalone_web.py           # 独立 HTTPS 静态服务与受限代理
├── memory/                     # 长期记忆 Memory 后端与迁移
├── script/
...
仓库保留 AGPL-3.0 许可证文本。长期记忆后端的上游归属、兼容名称和许可证说明见 [NOTICE](NOTICE)。

## 致谢

服务器信息查询和图片展示功能基于早期 `mcgetter` 实现继续修改；长期记忆能力来自 NOTICE 中列明的上游项目。感谢原作者及相关开源项目。
