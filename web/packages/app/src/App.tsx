...
function AppContent() {
  ...
  const controls = <div className="dashboard-controls">{mcRoute ? <SelectField className="wf-select-field--inline" id="global-group" label="group_id" options={groups.map((group) => ({ value: group.group_id, label: `${group.label} (${group.group_id})` }))} value={selectedGroupId} placeholder="—" disabled={loading || groups.length === 0 || settingsNavigationLocked} onChange={selectGroup} /> : null}<button className="wf-button wf-button--quiet theme-button" type="button" onClick={() => setTheme((value) => value === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? t('light') : t('dark')}</button></div>;
  ...
  return <I18nContext.Provider value={{ locale, t }}><WorkshopShell brand="Zhouyi Dashboard" groupControl={controls} navigation={<>{memoryAvailable ? <><NavLink to="/memory/overview">{t('overview')}</NavLink><NavLink to="/memory/memories">{t('memories')}</NavLink><NavLink to="/memory/recall">{t('recall')}</NavLink><NavLink to="/memory/graph">{t('graph')}</NavLink></> : null}<NavLink to="/mc/servers">{t('servers')}</NavLink><NavLink to="/mc/trends">{t('trends')}</NavLink><NavLink to="/mc/settings">{t('settings')}</NavLink></>}>
    ...
    {!loading && !error ? <>{!standalone && !memoryAvailable && bootstrap?.capabilities.memory.enabled ? <p className="capability-banner">{t('memoryUnavailable')}: {bootstrap.capabilities.memory.error ?? bootstrap.capabilities.memory.reason ?? t('notInitialized')}</p> : null}<Routes><Route path="/mc/servers" element={selectedGroupId ? <ServersPage /> : <DataState state="empty" title={t('empty')} message="group_id" />} /><Route path="/mc/trends" element={selectedGroupId ? <TrendsPage /> : <DataState state="empty" title={t('empty')} message="group_id" />} /><Route path="/mc/settings" element={selectedGroupId ? <SettingsPage onNavigationLockChange={setSettingsNavigationLocked} /> : <DataState state="empty" title={t('empty')} message="group_id" />} />{memoryAvailable ? <><Route path="/memory/overview" element={<OverviewPage />} /><Route path="/memory/memories" element={<MemoriesPage />} /><Route path="/memory/recall" element={<RecallPage />} /><Route path="/memory/graph" element={<Suspense fallback={<DataState state="loading" title={t('loading')} message="Graph module" />}><GraphPage /></Suspense>} /></> : null}<Route path="*" element={<Navigate replace to={defaultPath} />} /></Routes></> : null}
