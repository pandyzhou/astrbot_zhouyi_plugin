...
def validate_config(raw_config: dict[str, Any]) -> MemoryConfig:
    """
    验证并返回规范化的配置对象。
...
def validate_runtime_config_changes(
    current_config: MemoryConfig, changes: dict[str, Any]
) -> bool:
    """
    验证运行时配置更改是否有效。
